...
For those who have never used R before, In R we need to store the dataset (also reffered to as dataframe) into a variable in order to be able to use the dataset. the `<-` is an assignment operation, `=` can also be used.
...
```{r}
RStata::stata("summarize", data.in = df2)
RStata::stata("ta species", data.in = df2)
RStata::stata("codebook", data.in = df2)
```
...
```{r}
df2 <- RStata::stata("gen sepalarea=sepallength*sepalwidth", data.in=df2, data.out=TRUE)
knitr::kable(df2)
```
